...
public class GetOrderDetailsQueryHandler(IApplicationDbContext context, ICurrentUser ICurrentUser) : IRequestHandler<GetOrderDetailsQuery, GetOrderDetailsResult>
{
    ...
    {
        GetOrderDetailsResult? orderDetails = await context.Orders.Where(o => o.Id == query.OrderId && o.UserId == ICurrentUser.Id).Select(o => new GetOrderDetailsResult
        {
            OrderId = o.Id,
        ...
        if (orderDetails is null) throw new NotFoundException("Order", query.OrderId);
